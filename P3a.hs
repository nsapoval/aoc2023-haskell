...
getPartIndices = getFullIndices partCriteria 0 
...
getPartNeighborhoods s = concatMap (addNeighborhood (length s)) . getPartIndices $ s

computeFinalValue :: [String] -> Int
computeFinalValue s = sum . catMaybes $ zipWith (\x y -> if any (`elem` neighborhoods) x then Just y else Nothing) ranges values where 
